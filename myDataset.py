...
class CIFAR10Data(Dataset):
    ...
    def __getitem__(self, idx):
        self.f_in.seek(idx*3073)
        databuf = self.f_in.read(3073)

        label = int.from_bytes(databuf[0:1], byteorder="little")
        ...
        for ch in range(3):
            for i in range(32):
                for j in range(32):
                    mybyte = databuf[1 + 32*32*ch+32*i+j : 1 + 32*32*ch+32*i+j + 1]
